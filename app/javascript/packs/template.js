...
const addCare = () =>
	$("#btn-care-add").on("click", function () {
		let userId = $(this).attr("data-user");
		let houseId = $(this).attr("data-house");
		let hostId = $(this).attr("data-host");
		...
		window
			...
			.then((res) => res.json())
			.then((response) => {
				let boxContentCare = $("#box-content-care")
					.html(`<div id="btn-care-remove" data-host="${hostId}" data-care="${response.id}" data-user="${response.user_id}" data-house="${response.house_id}" class="btn btn-danger">
...
const removeCare = () =>
	$("#btn-care-remove").on("click", function () {
		let userId = $(this).attr("data-user");
		let houseId = $(this).attr("data-house");
		let careId = $(this).attr("data-care");
		let hostId = $(this).attr("data-host");
		window
			...
			.then((response) => {
				let boxContentCare = $("#box-content-care")
					.html(`<div id="btn-care-add" data-host="${hostId}" data-user="${userId}" data-house="${houseId}" class="btn btn-primary">
...
function socketNotification() {
	let badgeNoti = $("#badge-notification");
	let arrImage = [
		"https://i.imgur.com/uIgDDDd.jpg",
		"https://img.icons8.com/flat_round/64/000000/vote-badge.png",
	];
	let boxNoti = $(".my-notification-box");
	window
		.fetch(
			"http://localhost:3000/users/notifications/" +
				badgeNoti.attr("data-user"),
			{
				method: "GET",
			}
		)
		...
		.then(({ notifications, count }) => {
			let boxContentCare =
				"<h3 style='color: #212121; padding: 10px;'>Thông báo</h3>";
			for (let ix = 0; ix < notifications.length; ++ix) {
				if(notifications[ix].status == "none") {
					boxContentCare += `<div data-noti="${notifications[ix].id}" class="notifications-item"> <img src="${
						arrImage[parseInt(badgeNoti.attr("data-user")) % 2]
					}" alt="img">
															<div class="text">
																<h4>FHouse Admin</h4>
																<p>${notifications[ix].title}</p>
															</div>
															<span class="badge badge-danger badge-status">Mới</span>
														</div>`;
				} else {
					boxContentCare += `<div data-noti="${notifications[ix].id}" class="notifications-item"> <img src="${
						arrImage[parseInt(badgeNoti.attr("data-user")) % 2]
															...
															<span class="badge badge-dark badge-status">Đã xem</span>
														</div>`;
				}
			}
			boxNoti.html(boxContentCare);
			badgeNoti.html(count);
			if (count == 0) {
				badgeNoti.addClass("display_none");
			} else {
				badgeNoti.removeClass("display_none");
			}
...
function updateStatusNotification() {
	$(".notifications-item").click(function() {
		window
			.fetch("http://localhost:3000/users/notifications/update/" + $(this).attr("data-noti"), {
				method: "GET",
			})
			.then((res) => res.json())
			.then(({ notification }) => {
				console.log("Update okie")
			})
			.catch((e) => console.log(e));
	})
}
